...
def run_algorithm():
    ...
    for i in range(number_of_pictures):
        ...
        for _ in range(iter_per_picture):
        ...
        if TRACK_PHI_A:
            ...
            print(f"Picture {i} - Modified A-optimality target function: {round(phi_A_d, 3)} - Radiation boundary satisfied: {np.sum(1 / (d ** 2)) <= D} - Dose of radiation: {round(np.sum(1 / (d ** 2)), 3)}")
        ...
        # print the angle of largest d
        angle_offset_pair_index = np.argmax(d)
        print(f"Angle of the largest intensity: {offset_angle_pairs[angle_offset_pair_index][1] * 180 / np.pi}")
...
def plot_variance(gamma_posterior, N):
    ...
    fig, ax = plt.subplots()
    im = ax.imshow(variances, cmap='viridis', interpolation='nearest')
    fig.colorbar(im, ax=ax)
